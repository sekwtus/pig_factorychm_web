...
{
    function checking_order(){
        return view('transport.checking_order');
      }

    public function report_transport($order_number){
     ...
     // ใช้หลังวันที่22/04/2020
      if (substr($date_transport,6,4).substr($date_transport,3,2).substr($date_transport,0,2) > 20200422) {
          return $this->report_transport2($order_number);
      }
      ...
      if ( substr($date_transport,6,4).substr($date_transport,3,2).substr($date_transport,0,2) > 20200327 ) {
        ...
        return view('transport.report_transport',compact('select_item_main','select_weight_in_order','rowspan_count_item','select_cutting_number','report_transport_check',
    ...
    public function report_transport2($order_number){
      ...
      if ( substr($date_transport,6,4).substr($date_transport,3,2).substr($date_transport,0,2) > 20200327 ) {
        ...
        return view('transport.new_transport.report_transport',compact('select_item_main','select_weight_in_order','rowspan_count_item','select_cutting_number','report_transport_check',
          'wg_scale','wg_scale_shop','wg_weight_type','wg_sku','wg_storage','cutting_number',
          'offal_number','order_recieve','select_item_offal_main','select_weight_in_order_offal',
    ...
    public function check_report(Request $request){
      ...
      for ($i=0; $i <count($request->code_check) ; $i++) { 
        $unit_check = $request->unit_check[$i];
        $code_check =$request->code_check[$i];
        DB::update("UPDATE tb_report_transport_check set check_unit = '$unit_check'
        where item_code = '$code_check' AND order_number = '$request->tr_number' ");
      }
