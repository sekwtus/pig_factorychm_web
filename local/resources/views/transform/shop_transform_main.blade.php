    ...
    <div class="col-lg-12 grid-margin">
        <div class="card">
            ...
            <div class="row ">
                <div class="col-6">
                    <table class="table table-striped table-bordered nowrap table-hover table-responsive"  width="100%" id="orderTableLot">
                        <thead class="text-center">
                            <tr>
                            <th style="padding: 0px;min-width:10%; font-size: 0.7rem;">สาขา</th>
                            <th style="padding: 0px;min-width:10%; font-size: 0.7rem;">รหัสสาขา</th>
                ...
                <div class="col-6">
                    รายงานการแปรสภาพ ประจำวัน  <button title="รายงานแปรสภาพ"
                    style="padding: 5px;" onclick="goToLink()" type="button" class="btn btn-primary ">รายงานแปรสภาพ</button>
                </div>
            </div>
...
<script>
    function tableOrderLot(){
        var table2 = $('#orderTableLot').DataTable({
            ...
            // serverSide: true,
            ajax: {
                    url: '{{ url('get_data_shop_tranform_main') }}',
                    data: {},
                },
            columns: [
               
                { data: 'location_scale', name: 'location_scale' },
                { data: 'scale_number', name: 'scale_number' },
            ...
            columnDefs: [
                ...
                },
                {
                    "targets": 3,render(data,type,row){
                        var sign = "'";
                        var date_format = row['date'];
                        var scale_number = row['scale_number'];

                        return '<a target="blank_" href="transform/transform_compare/'+date_format+'/'+scale_number+'"><button title="รายงานแปรสภาพ"\
...
function goToLink() {
    var date = $("#daterangeLot").val();
    var var_format = date.substring(0,2)+date.substring(3,5)+date.substring(6,10);
    window.open("transform/transform_compare_all/"+var_format);
}
