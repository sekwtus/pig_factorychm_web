...
<div class="row ">
    <div class="col-lg-5 grid-margin">
        <div class="card">
            <div class="card-body">
                {{ Form::open(['method' => 'post' , 'url' => '/create_bill_iv']) }}
                    ...
                    <div class="row" style="margin-bottom: 10px;">
                        <div class="col-md-6 pr-md-0">
                            <label><b>เลขที่ใบสั่งของ</b></label>
    ...
    <div class="col-lg-7 grid-margin">
        <div class="card">
            <div class="card-body ">
                <div class="row">
                    <div class="col-4"><h4>รายการใบแจ้งหนี้</h4></div>
                    ...
                    <table class="table" width="100%" id="orderTable">
                        <thead class="text-center">
                            <tr>
                                <th style="padding: 0px; font-size: 0.7rem;">เลขที่ใบแจ้งหนี้</th>
                                <th style="padding: 0px; font-size: 0.7rem;">รหัสลูกค้า</th>
                                <th style="padding: 0px; font-size: 0.7rem;">วันที่</th>
                                <th style="padding: 0px; font-size: 0.7rem;">พิมพ์</th>
...
<script>
        var table = $('#orderTable').DataTable({
            ...
            },
            {
                "targets": 3,
                    "className": "text-center",render(data,type,row){
                        r = ''
                        s = "'";
                        r = r+'<a class="btn btn-success" style="margin-bottom: 10px;" title="พิมพ์บิลรวม" onclick="goto_print('+s+data+s+',1,1)" >\
            ...
            //                 pay = row['pay'];
            //             }
            //             if(row['total_price'] == row['pay']){
            //                 r = r+'<a class="btn btn-success" style="margin-bottom: 10px;" title="พิมพ์ใบเสร็จ" onclick="goto_print('+s+data+s+',5,1)" >\
            //                                                 <i class="mdi mdi-printer"></i></a> ';
            //             }else{
...
<script language="javascript" type="text/javascript">
    ...
    function goto_print(id,type,bill){
        // alert(bill);
        var r = confirm("ถ้ากดพิมพ์แล้วระบบจะสร้างบันทึกการรับเงินจะไม่สามารถแก้ไข้ได้หรือลบได้ ต้องการพิมพ์หรือไม่!");
        if(r == true){
            if(type == 0){
                window.open("{{ url('bill_detail') }}/"+id+"/"+bill, '_blank');
            }
            if(type == 1){
                window.open("{{ url('bill_detail_rc') }}/"+id+"/"+bill, '_blank');
            }
            if(type == 2){
            ...
            if(type == 5){
                window.open("{{ url('bill_detail_re') }}/"+id+"/"+bill, '_blank');
            }
        }
    }
    ...
    function get_order(id){
        ...
        if (id != '' ) {   
            $.ajax({
                ...
                url: '{{ url('get_order_bill_iv') }}',
                data: {id:id,date:date},
                success: function (data) {
                    console.log(data);
                    option = "";
                    data.forEach(element => {
                        option = option + '<option value="'+element.ivm_id+'">'+element.ivm_id+'</option> ';
                    });
                    $("#order").append(option);
                    $order_do = option;
                },
