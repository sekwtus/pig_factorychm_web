...
<tr>
    ...
    <td style="padding: 0px; height: 20px;" class="text-center" rowspan="{{ $span+($group_row_span2[$key]->count_row *2) }}">{{ $item_mian->yield_main }}</td>

    <td style="padding: 0px; height: 20px; background-color:#ff8080;" class="text-center" rowspan="{{ $span+($group_row_span2[$key]->count_row *2) }}">
    ...
    <td style="padding: 0px; height: 20px;" class="text-center" rowspan="{{ $span+($group_row_span2[$key]->count_row *2) }}">{{ ($item_mian->yield_per_one == null ? '' : number_format( $item_mian->yield_main/$item_mian->yield_per_one , 2, '.', '') ) }}</td>

    {{-- @foreach ($sp_summary_weight as $summary_weight)
        @if ($item_mian->item_code2 == $summary_weight->item_code2)
        <td style="padding: 0px; height: 20px; background-color:#80ff80;" class="text-center" rowspan="{{ $span+($group_row_span2[$key]->count_row *2) }}" >
    ...
    {{-- @foreach ($final_weight as $final_weights)
        ...
        <td style="padding: 0px; height: 20px;" class="text-center" rowspan="{{ $span+($group_row_span2[$key]->count_row *2) }}">
    ...
    @if ($check_weight == 0)
            ...
            @foreach ($shop_request_fill as $request_fill)
            @if ($request_fill->item_code == $item_mian->item_code)
            <td style="padding: 0px; height: 20px; background-color:#ff8080;" class="text-center" rowspan="{{ $span+($group_row_span2[$key]->count_row *2) }}" >{{ $request_fill->weight }}</td>
                @php
                    $check_fill_exist2++;
            ...
            <td style="padding: 0px; height: 20px; background-color:#9fff9f;" class="text-center" rowspan="{{ $span+($group_row_span2[$key]->count_row *2) }}" > {{ $item_mian->yield_main*$sum_number_of_pig }}</td>
            @php
                $weight_balance = $item_mian->yield_main*$sum_number_of_pig;
            @endphp
            @endif

        <td style="padding: 0px; height: 20px;" class="text-center" rowspan="{{ $span+($group_row_span2[$key]->count_row *2) }}">{{ ($item_mian->yield_per_one == null ? '' :number_format( $item_mian->yield_main*$sum_number_of_pig / $item_mian->yield_main/$item_mian->yield_per_one , 2, '.', '') ) }}</td>
    @endif
    ...
    <td style="padding: 0px; height: 20px;" class="text-center">% 
        <button type="submit" name="save_percent" value="1" class="btn btn-success mr-2" style=" padding: 0px;  height: 22px;">save</button>
    </td>
</tr>
