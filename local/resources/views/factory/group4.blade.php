 ...
 <tr>
     ...
     {{-- ไม่มีในorderพิเศษ --}}
    <td style="padding: 0px; height: 20px; background-color:#c0c0c0;" class="text-center" rowspan="3" ></td>
    <td style="padding: 0px; height: 20px; " class="text-center" rowspan="3" ></td>
    ...
    <td style="padding: 0px; height: 20px;" class="text-center" colspan="{{ count($shop_list)+2 }}" hidden></td>
        ...
        <tr>
                    ...
                    @if ($data_4->item_code2 == $item_mian->item_code2)
                        ...
                        @for ($i = 0; $i < count($shop_list); $i++)  @php $mk_code = $shop_list[$i]->shop_code; @endphp
                            <td style="padding: 0px; height: 20px;" class="text-center"> 
                                <input type="text" style="width: 35px;" name="number_item[{{ $mk_code }}][{{ $item_mian->item_code2 }}]"
                                        value="{{ $data_4->$mk_code }}" />
            ...
            @else
                ...
                @for ($i = 0; $i < count($shop_list); $i++)  @php $mk_code = $shop_list[$i]->shop_code; @endphp
                    <td style="padding: 0px; height: 20px;" class="text-center"> 
                        {{-- {{ $shoparray[$shop_request_list_group->order_special_id][$i] }} --}}
                        <input type="text" style="width: 35px;" name="number_item[{{ $mk_code }}][{{ $item_mian->item_code2 }}]"
                                value="0" />
