...
@section('main')  
  <div class="col-12 grid-margin">
    <form action="{{ url('/salepig_edit') }}" method="post">
    {{ csrf_field() }}
      <div class="card">
        <div class="card-body">
                  ...
                  <div class="card-body">
                    <div class="div-tbl-detail table-responsive">
                      <table class="table table-bordered">
                        ...
                        <tbody>
                          @foreach ($listpig as $i => $salepig)
                          <tr class="tr-detail tr-{{$salepig->list_id}}">
                            <td class="text-center no w60">{{$i+1}}</td>
                            <td>
                              <input type="text" name="txtCustomer[{{$salepig->list_id}}]" value="{{$salepig->customer_name}}" class="form-control w80 txtCustomer" onfocusout="validate1(this.name)">
                            </td>
                            <td>
                              <input type="text" name="txtAmount[{{$salepig->list_id}}]" value="{{$salepig->amount}}"style="width:135px" class="form-control txtAmount" onfocusout="validate1(this.name)">
                            </td>
                            <td>
                              <input type="text" name="txtWeight[{{$salepig->list_id}}]" value="{{$salepig->weight_range}}" class="form-control w100 txtWeight" onfocusout="validate1(this.name)">
                            </td>
                            <td class="jsgrid-cell">
                              <input type="text" name="txtNote[{{$salepig->list_id}}]" value="{{$salepig->note}}" class="form-control w80 txtNote" onfocusout="validate1(this.name)">
                            </td>
                            
                            <td class="text-center w60">
                              <a style="cursor:pointer; color:red;" title="ลบข้อมูล" onclick="delete_detail({{$salepig->list_id}})"><i class="mdi mdi-36px mdi-close-outline"></i></a>
                            </td>
                          </tr>
                          @endforeach
                          <tr class="tr-btn">
...
<script>
  ...
  function runnumber(){ 
      ...
      if(row==0){
        ...
        for(i=0; i<row; i++){
          ...
          $('.tr-detail-new .txtWeight').eq(i).attr({name:'txtWeight['+k+']', onfocusout:'validate1(this.name)'});
  ...
  if (result.value) {
    $.ajax({
          type: 'GET',
          data: {id : id},
          url: '{{url('/salepig/deleteDetail/')}}', //billno  OR bill/gen_billno.php
