...
@section('main')  
  <div class="col-12 grid-margin">
    <form action="{{ url('/saleslice_add') }}" method="post">
    {{ csrf_field() }}
      <div class="card">
        <div class="card-body">
...
<script>
  ...
  function runnumber(){
      ...
      if(row==0){
        ...
        for(i=0; i<row; i++){
          ...
          $('.sliceType').eq(i).attr({name:'sliceType['+n+']', onfocusout:'validate1(this.name)'});
  ...
  $('.btn-add').click(function() { alert(555)
    ...
    for(i=0; i < row; i++){
      ...
      if($('.sliceType').eq(i).val()=='0'){
        $(this).attr('disabled',true);
        $('.sliceType').eq(i).css('border-color','red');
      }
      if($('.txtNote').eq(i).val()==''){
        $(this).attr('disabled',true);
