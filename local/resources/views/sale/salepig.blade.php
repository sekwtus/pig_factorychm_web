    ...
    <div class="card-body">
      ...
      <div class="row mb-3">
        ...
        <div class="col-6 text-right">
          <a href="{{url('/salepig/add')}}" class="btn btn-success btn-lg" >
              ...
              <div class="col-12">
                <table class="table table-striped dataTable- no-footer- " id = "example">
                    ...
                    <tbody class="text-center">
                      @foreach ($listpig as $salepig)
                        <tr>
                          <td> {{ $salepig->sale_no }} </td>
                          <td>{{ $salepig->date }}</td>
                          <td>{{ $salepig->total }}</td>
                          <td class="text-center">
                            <a class="btn btn-success btn-fw" href="{{url('/salepig/report') }}" >เอกสาร</a>
                          </td>
                          <td class="text-center">
                            <a class="btn btn-info " href="{{ url('salepig/view/'.$salepig->sale_no) }}">
                              <i class="mdi mdi-eye"></i> เรียกดู
                            </a>
                            <a class="btn btn-warning "  href="{{url('/salepig/edit/'.$salepig->sale_no) }}">
                              <i class="mdi mdi-wrench"></i> แก้ไข
                            </a>
                            {{-- <a class="btn btn-warning" href="#" ><i class="mdi mdi-wrench"></i>แก้ไข</a>&nbsp;&nbsp; --}}
                            <button class="btn btn-danger "  data-toggle="modal"  type="button" data-target="#DELETE{{ $salepig->sale_no }}">
      ...
      {{-- ลบข้อมูล --}}
      @foreach($listpig as $salepig)
        {{ Form::open(['method' => 'post' , 'url' => '/salepig/delete/'.$salepig->sale_no]) }}
          <div class="modal fade" id="DELETE{{ $salepig->sale_no }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                                <h5 >ยืนยันการลบ ใบสั่งแผนเลขที่ {{ $salepig->sale_no }}</h5>
                        </div>
