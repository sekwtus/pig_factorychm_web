...
@section('main')  
  <div class="col-12 grid-margin">
    <form action="{{ url('/saleslice_edit') }}" method="post">
    {{ csrf_field() }}
      <div class="card">
        <div class="card-body">
          <h4>เพิ่มใบสั่งแผน Sale Order เชือดชำแหละ</h4>
          <div class="collapse show" role="tabpanel" data-parent="#accordion">
            <div class="form-group">
                <label for="exampleInputNo" name="no" >เลขที่</label>
            <input type="text" name="no" value = "{{$no}}" class="form-control txtNo" placeholder="กรอกเลขที่" onfocusout="validate1(this.name)" readonly> 
            </div>
                      ...
                      <table class="table table-bordered">
                        <thead class="text-center thead-light">
                          <tr> 
                            ...
                            <th>จำนวนตัว</th>
                            <th>ประเภทการเชือด</th>
                            <th>หมายเหตุ</th>
                            <th>ลบ</th>
                          </tr>
                        </thead>
                        <tbody>
                          @foreach ($listpig as $i => $saleslice)
                          <tr class="tr-detail tr-{{$saleslice->list_id}}">
                            <td class="text-center no w60">{{$i+1}}</td>
                            <td>
                              <input type="text" name="txtCustomer[{{$saleslice->list_id}}]" value="{{$saleslice->customer_name}}" class="form-control w80 txtCustomer" onfocusout="validate1(this.name)">
                            </td>
                            <td>
                              <input type="text" name="txtAmount[{{$saleslice->list_id}}]" value="{{$saleslice->amount}}"style="width:135px" class="form-control txtAmount" onfocusout="validate1(this.name)">
                            </td>
                            <td>
                              <select class="form-control sliceType" name="sliceType[{{$saleslice->list_id}}]">
                                <option value="{{$saleslice->slice_type}}">{{$saleslice->slice_type}}</option> 
                                <option value="C1">C1</option><option value="C2">C2</option><option value="C3">C3</option> 
                                <option value="อื่นๆ">อื่นๆ</option>  
                              </select>                           
                            </td>
                            <td class="jsgrid-cell">
                              <input type="text" name="txtNote[{{$saleslice->list_id}}]" value="{{$saleslice->note}}" class="form-control w80 txtNote" onfocusout="validate1(this.name)">
                            </td>
                            
                            <td class="text-center w60">
                              <a style="cursor:pointer; color:red;" title="ลบข้อมูล" onclick="delete_detail({{$saleslice->list_id}})"><i class="mdi mdi-36px mdi-close-outline"></i></a>
                            </td>
                          </tr>
                          @endforeach
                          <tr class="tr-btn">
...
<script>
  ...
  function runnumber(){ 
      ...
      if(row==0){
        ...
        for(i=0; i<row; i++){
          ...
          $('.tr-detail-new .sliceType').eq(i).attr({name:'sliceType['+k+']', onfocusout:'validate1(this.name)'});
  ...
  function add_row(){
      ...
      row += '<td><select class="form-control sliceType" name="sliceType[]"><option value="0">เลือกประเภทการเชือด</option> <option value="C1">C1</option><option value="C2">C2</option><option value="C3">C3</option> <option value="อื่นๆ">อื่นๆ</option>  </select></td>';                                                              
 ...
 if (result.value) {
   $.ajax({
         type: 'GET',
         data: {id : id},
         url: '{{url('/saleslice/deleteDetail/')}}', //billno  OR bill/gen_billno.php
